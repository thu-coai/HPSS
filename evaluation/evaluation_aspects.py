...
def correlation_summary(v, u, strip=16):
    ...
    return {
        "Pearson" : p / max(length, 1),
        "Spearman" : s / max(length, 1),
        "Kendall" : k / max(length, 1)
    }
...
if __name__ == "__main__":
    ...
    for aspect in dataset2aspects[args.dataset]:
        with open(os.path.join(args.input_path, aspect + "_" + args.file_name + ".jsonl"), "r", encoding='utf-8') as f:
        ...
        for i, d in enumerate(data):
            ...
            human_scores.append(d['scores'][aspect])
            d[f'{aspect}_score'] = score
    ...
    result['average'] = {
        "Pearson" : average["Pearson"] / len(dataset2aspects[args.dataset]),
        "Spearman" : average["Spearman"] / len(dataset2aspects[args.dataset]),
        "Kendall" : average["Kendall"] / len(dataset2aspects[args.dataset])
    }
